...
> 文档版本：v1.1 | 最后更新：2026-09-16 | 当前进度：节点 4/4（已完成公网部署与验收）
...
| 开发类型 | 纯前端静态网站 |
| 当前状态 | 已部署至阿里云 ECS，并完成公网桌面/平板/移动端验收 |
...
- [ ] 使用 SSH 密钥替代 root 密码登录，并在验证密钥可用后关闭密码登录。
...
| 服务器免密 SSH 验证失败 | 已绕过 | 由用户在现有终端完成密码认证并执行部署命令；未把密码写入任何配置或仓库 |
| TCP 80 被旧 Python 静态服务占用 | 已解决 | 终止 `/usr/bin/python3 -m http.server 80` 后启动 Nginx；首页与文章页均返回 200 |

## 十二、生产验收记录

验收日期：2026-09-16。

- 公网首页 `http://123.56.190.100/`：200。
- 公网文章页 `/articles/evidence-driven-ai-infra.html`：200。
- 首页文章卡片可正常跳转至文章页。
- 首页和文章页无浏览器控制台错误，CSS 静态资源加载成功。
- 375×812、768×1024、1280×720 三种视口均完成视觉检查。
