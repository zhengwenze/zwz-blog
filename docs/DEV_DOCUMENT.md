...
> 文档版本：v1.3 | 最后更新：2026-09-17 | 当前进度：CI/CD 实现、安全加固与生产端到端验收已完成
...
| 开发类型 | 纯前端静态网站 |
| 网站状态 | 已部署至阿里云 ECS |
| CI/CD 状态 | PR CI、`main` 自动发布、手动回滚、失败恢复和并发串行验收已完成 |
...
2026-09-17 已验证生产 Nginx 新缓存策略生效：首页、文章页、CSS 和部署元数据均返回 200，`deploy-meta.json` 响应包含 `Cache-Control: no-cache`。

## 七、验收结果与证据边界

原有网站的公网首页、文章页与响应式视图已完成人工验收。新 CI/CD 也已在 GitHub Actions 和生产服务器上完成以下验收：

- [x] Pull Request 只运行 CI，不连接生产服务器。
- [x] `main` push 自动部署准确提交，线上 `deploy-meta.json.sha` 等于 GitHub 提交 SHA。
- [x] 首页、文章页、CSS 和元数据的本机/公网检查全部通过。
- [x] 在隔离测试中制造健康检查失败，证明自动回滚保留上一健康版本。
- [x] 手动回滚到上一 release，验证后再重新部署最新 SHA。
- [x] 同时触发两个生产任务，证明它们串行执行且不中断正在进行的切换。
...
| 场景 | 运行 ID | 结果 |
| --- | --- | --- |
| PR 只运行 CI | `35099021074` | CI 成功，`Deploy production` 明确为 skipped |
| `main` 自动发布 | `35099089706` | 发布 `a9af2227af1fef2a205ccd929cd568882402234f` 成功 |
| 手动回滚 | `35099247979` | 成功切换到 `829f8db5dcbab749c5e7b47ed8ce0ba3b0ae2361` |
| 并发部署 1 | `35168237009` | 生产阶段 `00:51:42–00:52:10Z` 成功 |
| 并发部署 2 | `35168235319` | 等待锁后于 `00:52:17–00:52:42Z` 成功，与前一次无重叠 |

重新部署后，生产 `current` 已恢复指向 `a9af2227af1fef2a205ccd929cd568882402234f`。同一不可变 release 再次激活时不改写其 `deploy-meta.json`，因此元数据保留首次构建该 release 的运行 ID `35099089706`。
...
| CI/CD 生产演练 | 已完成 | Actions 与 ECS 已完成正常发布、隔离失败恢复、手动回滚和并发串行测试 |
